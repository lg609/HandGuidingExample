...
double RobotControl::s_pos_wth = 0.03;
double RobotControl::s_pos_wcr = 0.14;
double RobotControl::s_pos_lambda = 1;

double RobotControl::s_ori_wth = 0.1;
double RobotControl::s_ori_wcr = 0.5;
double RobotControl::s_ori_lambda = 1;

bool RobotControl::enable_constraints = false;

static RobotControl *s_instance = 0;

double RobotControl::s_pose1_calibration[6] = {0};
double RobotControl::s_pose2_calibration[6] = {0};
double RobotControl::s_pose3_calibration[6] = {0};
...
RobotControl::RobotControl():tcp2canMode_(false),IO_name_("0")
{
...
bool RobotControl::initRobotService()
{
    ...
    //tool dynamical parameters -> set by user
...
void RobotControl::startHandGuiding()
{
    ...
    while(s_thread_handguiding)
    {
        ...
        if(s_start_handguiding && IOSwitch == 1)
        {
            ...
            if(addSize > 0)
            {
                ...
                for(int bufferCount = 1; bufferCount <= addSize; bufferCount++)
                {
                    ...
                    for(int i = 0; i < aubo_robot_namespace::ARM_DOF; i++)
                    {
                        ...
                        average_sensor_data_[i] = force_of_end_[i] * 0.6 + average_sensor_data_[i] * 0.4;
                    }
                    ...
                    if(FTSensorDataProcess::s_controlSpace == CONTROL_SPACE::JOINTSPACE)
                    {
                        ...
                        for(int i = 0; i < aubo_robot_namespace::ARM_DOF; i++)
                        {
                            current_joint_dd_[i] = 1.0/FTSensorDataProcess::s_mass[i] * (jointTorque[i] - FTSensorDataProcess::s_damp[i] * current_joint_d_[i] - FTSensorDataProcess::s_stiffness[i] * last_send_joints_[i]);   //a = k*force   Newton
                    ...
                    else
                    {
                        for(int i = 0; i < aubo_robot_namespace::ARM_DOF; i++)
                        {
                            ...
                            if(FTSensorDataProcess::s_controlModel == CONTROL_MODE::ACCLERATION)
                            {
                                current_acclerations_[i] = 1.0/FTSensorDataProcess::s_mass[i] * (average_sensor_data_[i] - FTSensorDataProcess::s_damp[i] * last_velocities_[i] - FTSensorDataProcess::s_stiffness[i] * relative_position_[i]);   //a = k*force   Newton
                                current_velocities_[i] = last_velocities_[i] +  s_control_period * (current_acclerations_[i] + last_acclerations_[i]) / 2000.0;
                            }
                            else
                                //                            current_velocities_[i] = FTSensorDataProcess::s_mass[i] * average_sensor_data_[i];
                                current_velocities_[i] = (average_sensor_data_[i] + CONTROL_CYCLE * FTSensorDataProcess::s_mass[i] * last_velocities_[i]) / (FTSensorDataProcess::s_mass[i] * CONTROL_CYCLE + FTSensorDataProcess::s_damp[i]);
                        }
...
bool RobotControl::calculateTheoreticalWaypoint()
{
    ...
    //limit
    double posNorm = sqrt(relative_pose_.relativePosition[0]*relative_pose_.relativePosition[0]+relative_pose_.relativePosition[1]*relative_pose_.relativePosition[1]
            +relative_pose_.relativePosition[2]*relative_pose_.relativePosition[2]);
    if(posNorm > (max_vel_*0.005))
    {
        double scale = posNorm /(max_vel_*0.005);
        relative_pose_.relativePosition[0] /= scale;
        relative_pose_.relativePosition[1] /= scale;
        relative_pose_.relativePosition[2] /= scale;
    }

    double oriNorm = sqrt(relative_axis_angles_[0]*relative_axis_angles_[0]+relative_axis_angles_[1]*relative_axis_angles_[1]
            +relative_axis_angles_[2]*relative_axis_angles_[2]);
    if(oriNorm > (max_rot_*0.005))
    {
        double scale = oriNorm /(max_rot_*0.005);
        relative_axis_angles_[0] /= scale;
        relative_axis_angles_[1] /= scale;
        relative_axis_angles_[2] /= scale;
    }
...
{
    aubo_robot_namespace::RobotDiagnosis robot_diagnosis_info_;
    robotServiceReceive.robotServiceGetRobotDiagnosisInfo(robot_diagnosis_info_);
    if(robot_diagnosis_info_.robotCollision)
    {
        enterTcp2CANMode(false);
        emit signal_handduiding_failed("collision.");
    }
    if(IO_name_ == "0")
        IOSwitch = 1;
    else
        robotServiceReceive.robotServiceGetBoardIOStatus(aubo_robot_namespace::RobotBoardUserDI, IO_name_, IOSwitch);
    return IOSwitch;
}
...
bool RobotControl::ObtainCenterofMass()
{
    ...
    {
        pose_jointangle[0][i] = s_pose1_calibration[i];
        pose_jointangle[1][i] = s_pose2_calibration[i];
        pose_jointangle[2][i] = s_pose3_calibration[i];
    }
...
int RobotControl::moveToTargetPose(int index)
{
    ...
    double jointAngle[6];
    if(index == 1)
        memcpy(jointAngle, s_pose1_calibration, sizeof(double)*aubo_robot_namespace::ARM_DOF);
    else if(index == 2)
        memcpy(jointAngle, s_pose2_calibration, sizeof(double)*aubo_robot_namespace::ARM_DOF);
    else
        memcpy(jointAngle, s_pose3_calibration, sizeof(double)*aubo_robot_namespace::ARM_DOF);
...
void RobotControl::setCalibrationPose(int index, double joint_angle[])
{
    //move to target pose
    aubo_robot_namespace::wayPoint_S wayPoint;
    robotServiceSend.robotServiceGetCurrentWaypointInfo(wayPoint);
    if(index == 1)
        memcpy(s_pose1_calibration, wayPoint.jointpos, sizeof(double)*aubo_robot_namespace::ARM_DOF);
    else if(index == 2)
        memcpy(s_pose2_calibration, wayPoint.jointpos, sizeof(double)*aubo_robot_namespace::ARM_DOF);
    else
        memcpy(s_pose3_calibration, wayPoint.jointpos, sizeof(double)*aubo_robot_namespace::ARM_DOF);

    memcpy(joint_angle, wayPoint.jointpos, sizeof(double)*aubo_robot_namespace::ARM_DOF);
}
...
double RobotControl::getPerformanceIndex(const RVector q, RMatrix& J_inv, JACOBIAN_TYPE type)
{
...
void RobotControl::setHandGuidingSwitchIO(const std::string& IO_name__)
{
    IO_name_ = IO_name__;
}

void RobotControl::setHandGuidingMaxVel(const double value)
{
    max_vel_ = value;
}

void RobotControl::setHandGuidingMaxRot(const double value)
{
    max_rot_ = value;
}
